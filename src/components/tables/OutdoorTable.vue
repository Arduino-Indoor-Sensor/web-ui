...
import { customOutdoorData } from '../../utils/data';
  ...
  async mounted() {
    ...
    indoorNode.forEach((element) => {
      sensorNodeFilter.push({
		...
		let gatewayFilter = [];
    res = await this.$axios.get("/outdoor/gateway");
		let gateway = res.data.data;
    gateway.forEach((element) => {
      gatewayFilter.push({
        text: `网关${element.id}`,
        value: element.id,
      });
    ...
    const columns = [
      ...
      },
      {
        ...
        onFilter: (value, record) => record.gateway.slice(2) === String(value),
      },
      {
        title: "光强",
      ...
      },
      {
		...
		let data = [];
		res = await this.$axios.get("/outdoor/data");
		let indoorData = res.data.data;
		indoorData.forEach((element) => {
			data.push(customOutdoorData(element));
		})
